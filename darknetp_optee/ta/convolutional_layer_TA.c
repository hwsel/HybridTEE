...
void binarize_weights_TA(float *weights, int n, int size, float *binary)
{
    ...
    // Update TCB tracker
    gGlobalBufferTracker += (n*sizeof(float));
}
...
convolutional_layer_TA make_convolutional_layer_TA_new(int batch, int h, int w, int c, int n, int groups, int size, int stride, int padding, ACTIVATION_TA activation, int batch_normalize, int binary, int xnor, int adam, int flipped, float dot)
{
    ...
    // Update TCB tracker
    gGlobalBufferTracker += sizeof(convolutional_layer_TA);
    ...
    // Update TCB tracker
    gGlobalBufferTracker += ((c/groups*n*size*size) * sizeof(float));
    gGlobalBufferTracker += (n * sizeof(float));
    ...
    */
    ...
    // Update TCB tracker
    gGlobalBufferTracker += (2 * sizeof(int) + sizeof(float));
    ...
    // Update TCB tracker
    //gGlobalBufferTracker += (2 * l.batch*l.outputs * sizeof(float));
	...
	// Update TCB tracker
        gGlobalBufferTracker += ((l.nweights * sizeof(float)) + (l.nweights * sizeof(char)) + (n * sizeof(float)));
    }
    if(xnor){
        l.binary_weights = calloc(l.nweights, sizeof(float));
        l.binary_input = calloc(l.inputs*l.batch, sizeof(float));
	// Update TCB tracker
        gGlobalBufferTracker += ((l.nweights * sizeof(float)) + (l.inputs * l.batch * sizeof(float)));
    }
	...
	// Update TCB tracker
        gGlobalBufferTracker += ((5 * n * sizeof(float)) + (l.batch*l.outputs * sizeof(float)));
    }
    if(adam){
	...
	// Update TCB tracker
        gGlobalBufferTracker += ((4 * n * sizeof(float)) + (2 * l.nweights * sizeof(float)));
    }
...
void forward_convolutional_layer_TA_new(convolutional_layer_TA l, network_TA net)
{
    ...
    // Update TCB tracker
    //gGlobalBufferTracker += (16*l.batch*l.groups);
