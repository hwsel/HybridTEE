...
void forward_connected_layer_TA_new(layer_TA l, network_TA net)
{
    ...
    // Update TCB tracker
    gGlobalBufferTracker += (3*sizeof(int) + 12);
...
layer_TA make_connected_layer_TA_new(int batch, int inputs, int outputs, ACTIVATION_TA activation, int batch_normalize, int adam)
{
    ...
    // Update TCB tracker
    gGlobalBufferTracker += sizeof(layer_TA);
    ...
    // Update TCB tracker
    gGlobalBufferTracker += ((2*batch*outputs * sizeof(float)) + (2*inputs*outputs * sizeof(float)) + (2*outputs*sizeof(float)));
	...
	// Update TCB tracker
        gGlobalBufferTracker += ((2*l.inputs*l.outputs*sizeof(float)) + (4*l.outputs*sizeof(float)));
    }
	...
	// Update TCB tracker
        gGlobalBufferTracker += ((2*batch*outputs*sizeof(float)) + (8*outputs*sizeof(float)));
    }
