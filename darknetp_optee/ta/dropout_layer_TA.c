...
dropout_layer_TA make_dropout_layer_TA_new(int batch, int inputs, float probability, int w, int h, int c, int netnum)
{
    ...
    // Update TCB tracker
    gGlobalBufferTracker += sizeof(dropout_layer_TA);
    ...
    // Update TCB tracker
    gGlobalBufferTracker += ((3*inputs*batch*sizeof(float)) + (20*sizeof(char)));
...
void forward_dropout_layer_TA_new(dropout_layer_TA l, network_TA net)
{
    ...
    // Update TCB tracker
    gGlobalBufferTracker += ((l.batch * l.inputs * sizeof(float)) + sizeof(int) + 4);
}
