...
int parse_last_layer(char *filesize);
...
int parse_last_layer(char *filesize)
{
    //list *options = read_data_cfg(cfg);
    //int inputs = option_find_int(options, "inputs", 1);
    //int groups = option_find_int(options, "groups", 1);
    //int batch = option_find_int(options, "batch", 1);
    int inputs;
    int batch = 1;
    ...
    if(inputs <= 0)
    {
        printf("Error reading value from file\n");
        return 1;
    }

    inputs /= sizeof(float);
    inputs /= batch;
    memset((void *)&sl, 0, sizeof(layer));
    sl.inputs = inputs;
    sl.outputs = inputs;
    //l.groups = groups;
    sl.groups = 1;        // Standard value, load from config if needed ?
    sl.batch = batch;
    ...
    make_softmax_layer_CA(sl.batch, sl.inputs, sl.groups, sl.temperature, sl.w, sl.h, sl.c, sl.spatial, sl.noloss);
    return 0;
}
...
void predict_classifier(char *datacfg, char *cfgfile, char *weightfile, char *filename, int top)
{
	...
	float *partial_results = network_predict(net, X);
	printf("Done....\n");
        //layer l = net->layers[net->n - 1];
        int total_size = l.outputs * l.batch;
	...
	*/
	...
	/*
	// Corrupt the partial results for integrity testing
	printf("\nCorrupt the partial results for integrity testing....\n");
	...
	*/
	...
	char output_tag[80];
	strcpy(output_file, "data/darknet/partial_outputs_trustzone.data");
	fprintf(stderr, "Saving partial outputs to %s\n", output_file);
	strcpy(output_tag, "data/darknet/tag_trustzone.data");
        fprintf(stderr, "Saving generated GCM tag to %s\n", output_tag);
        FILE *fp = fopen(output_file, "wb");
	FILE *fp_1 = fopen(output_tag, "wb");
        if(!fp) file_error(output_file);
	if(!fp_1) file_error(output_tag);

	fwrite(partial_results, sizeof(float), total_size, fp);
	fwrite(net_tag_buffer, 1, 16, fp_1);

        free(net_output_back);
	// Reset the tag buffer for the final results
	...
	fclose(fp_1);
        ...
        // Send the partial results to SGX
        printf("\nSending partial results to SGX\n");
        int status = 0;
	...
	status = Socket_Client(output_file, output_tag, 5000, 0);
        if(status != 0)
        {
            printf("\nFile sending failed...\n");
            exit(1);
        ...
        fprintf(stderr, "Layers processed in %f seconds.\n", sec(clock()-time));

	// Wait for partial results from SGX
	printf("\nWait for partial results from SGX\n");
	status = Socket_Server("data/darknet/partial_outputs_sgx.data", "data/darknet/filesize.txt", "data/darknet/tag_sgx.data", "data/darknet/tag_size.txt", &time, 5000, 0);
        if(status != 0)
        {
	...
	// Perform the final prediction in trustzone
        //printf("\nPerform final prediction in trustzone\n");
	char *filesize = "data/darknet/filesize.txt";
	char *tag_buffer = "data/darknet/tag_sgx.data";
	//char *tag_size = "data/darknet/tag_size.txt";
        char *part_result = "data/darknet/partial_outputs_sgx.data";

	// Read the new tag into the local buffer
	FILE *fp1 = fopen(tag_buffer, "rb");
        if(!fp1) file_error(tag_buffer);
        fread(net_tag_buffer, 1, 16, fp1);

	printf("Create the softmax layer\n");
        status = parse_last_layer(filesize);

        if(status != 0)
	{
            printf("\nSoftmax layer creation failed...\n");
            exit(1);
	}

	printf("\nPredict final results from partial outputs\n");
	printf("Run softmax layer for prediction\n");
        float *predictions = network_predict_softmax(part_result);
        //if(net->hierarchy) hierarchy_predictions(predictions, net->outputs, net->hierarchy, 1, 1);

        top_k(predictions, sl.outputs, top, indexes);
        
	fprintf(stderr, "\nLayers processed in %f seconds.\n", sec(clock()-time));
...
void run_classifier(int argc, char **argv)
{
    ...
    global_dp = dp;
    int gpp = find_int_arg(argc, argv, "-gpp", 1);
    global_partition = gpp;
