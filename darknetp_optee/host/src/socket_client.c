...
int SocketConnect(int hSocket, int port)
{
    ...
    remote.sin_addr.s_addr = inet_addr("169.254.232.175"); //Server address
...
int Socket_Client(char *filename, char *filename1, int port, int mode)
{
    ...
    if(filename == NULL || ((mode == 0) && (filename1 == NULL)))
    {
        printf("\nIncorrect filename\n\n");
	return 1;
    }
    ...
    lin.l_onoff = 1;
    lin.l_linger = 1;
    if(setsockopt(hSocket, SOL_SOCKET, SO_LINGER, (const char *)&lin, sizeof(struct linger)) < 0)
