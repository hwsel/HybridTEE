...
void forward_network(network *netp)
{
	...
	if(1)
        {
            forward_network_CA(net.input, l.inputs, l.batch, net.train);
            i = net.n;
...
float *network_predict(network *net, float *input)
{
	...
	net_output_return_CA(l.outputs, l.batch);
        out = net_output_back;
    }
...
float *network_predict_softmax(char *data_file)
{
    float *out;
    float *input = (float *)malloc(sizeof(float) * sl.inputs * sl.batch);

    fprintf(stderr, "Loading partial outputs from %s...", data_file);
    fflush(stdout);
    FILE *fp = fopen(data_file, "rb");
    if(!fp) file_error(data_file);

    fread(input, sizeof(float), sl.inputs * sl.batch, fp);

    // Find the prediction of softmax layer
    net_result_back = malloc(sizeof(float) * sl.outputs * sl.batch);
    forward_softmax_CA(input, sl.inputs, sl.batch, sl.outputs, net_result_back);
