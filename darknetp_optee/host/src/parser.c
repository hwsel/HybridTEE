...
void parse_net_options(list *options, network *net)
{
    ...
    make_network_CA(net->n, net->learning_rate, net->momentum, net->decay, net->time_steps, net->notruth, net->batch, net->subdivisions, net->random, net->adam, net->B1, net->B2, net->eps, net->h, net->w, net->c, net->inputs, net->max_crop, net->min_crop, net->max_ratio, net->min_ratio, net->center, net->clip, net->angle, net->aspect, net->saturation, net->exposure, net->hue, net->burn_in, net->power, net->max_batches);
}
...
network *parse_network_cfg(char *filename)
{
    ...
    network *net = make_network(global_partition);
    ...
    while(n && (count < global_partition)){
        params.index = count;
	...
	allocate_workspace_CA((int)workspace_size);
...
void load_connected_weights_comm(layer l, FILE *fp, int i, int transpose)
{
    ...
    //printf("\n CONN layer = %d l_bias = %d l_weights = %d\n", i, l.outputs, l.outputs*l.inputs);
    if (l.batch_normalize && (!l.dontloadscales)){
        fread(l.scales, sizeof(float), l.outputs, fp);
        fread(l.rolling_mean, sizeof(float), l.outputs, fp);
        fread(l.rolling_variance, sizeof(float), l.outputs, fp);
...
void load_convolutional_weights_comm(layer l, FILE *fp, int i)
{
    ...
    //printf("\n CONV layer = %d l_bias = %d l_weights = %d\n", i, l.n, num);
    if (l.batch_normalize && (!l.dontloadscales)){
        fread(l.scales, sizeof(float), l.n, fp);
        fread(l.rolling_mean, sizeof(float), l.n, fp);
        fread(l.rolling_variance, sizeof(float), l.n, fp);
