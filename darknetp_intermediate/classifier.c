...
void predict_classifier(char *datacfg, char *cfgfile, char *weightfile, char *filename, int top)
{
	...
	int num_channels;
	// Darknet19
	int channels[8] = {32,32,64,64,128,64,128,128};
	// VGG-16
	//int channels[8] = {64,64,64,128,128,128,256,256};
	// Resnet152
	//int channels[8] = {64,64,64,64,256,256,64,64};
	// GoogLeNet
	//int channels[8] = {64,64,192,192,128,256,256,512};
	...
	{	
		...
		//else
		//	num_channels = 128;
		num_channels = channels[j];		

		for(int k = 0; k < num_channels; k++)
		{
			char im_name[256];
/*
			strcpy(im_name, input);
			strcat(im_name, "_");
			sprintf(im_name, "%s%d", im_name, j);
			strcat(im_name, "_");
			sprintf(im_name, "%s%d", im_name, k);
*/
			strcpy(im_name, input);
			char *match = im_name;
        		while ((match = strstr(match, ".jpg"))) {
            			*match = '\0';
            			strcat(im_name, match+4);
            			match++;
            			break;
        		}
			strcat(im_name, "_");
        		...
        		strcat(im_name, "_");
        		sprintf(im_name, "%s%d", im_name, k);
			image layer_image = get_network_image_layer(net, j, k);
			save_image(layer_image, im_name);
		}
	}
        if(r.data != im.data) free_image(r);
        free_image(im);
