...
image get_network_image_layer(network *net, int i, int j)
{
    layer l = net->layers[i];
#ifdef GPU
    //cuda_pull_array(l.output_gpu, l.output, l.outputs);
#endif
	// Create a new image for each channel and copy the contents of layer output at the respective channel offset to this image 
    if (l.out_w && l.out_h && l.out_c){
		if (j == -1)
			return float_to_image(l.out_w, l.out_h, l.out_c, l.output);
		image new_image = make_image(l.out_w, l.out_h, 1);
		memcpy(new_image.data, l.output + (j * l.out_w * l.out_h), l.out_w * l.out_h * sizeof(float));
        return new_image;
    }
    image def = {0};
    return def;
...
image get_network_image(network *net)
{
	...
	image m = get_network_image_layer(net, i, -1);
        if(m.h != 0) return m;
    }
    image def = {0};
    return def;
